...
// Wait for the EPGs and port groups to be created
resource "time_sleep" "wait_30_seconds" {
  depends_on      = [aci_application_epg.WEB_EPG, aci_application_epg.APP_EPG, aci_application_epg.DB_EPG]
  create_duration = "30s"
}

// Concatenate the tenant, app profile, and EPG to get the port group name
data "vsphere_network" "network_web" {
  name          = "${aci_tenant.tenant.name}|${aci_application_profile.test_app.name}|${aci_application_epg.WEB_EPG.name}"
  datacenter_id = data.vsphere_datacenter.dc.id
  depends_on    = [time_sleep.wait_30_seconds]
}

data "vsphere_network" "network_app" {
  name          = "${aci_tenant.tenant.name}|${aci_application_profile.test_app.name}|${aci_application_epg.APP_EPG.name}"
  datacenter_id = data.vsphere_datacenter.dc.id
  depends_on    = [time_sleep.wait_30_seconds]
}

data "vsphere_network" "network_db" {
  name          = "${aci_tenant.tenant.name}|${aci_application_profile.test_app.name}|${aci_application_epg.DB_EPG.name}"
  datacenter_id = data.vsphere_datacenter.dc.id
  depends_on    = [time_sleep.wait_30_seconds]
}
